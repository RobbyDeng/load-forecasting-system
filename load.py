...
def AMPDS(self, status):
    ...
    predict = np.around(pred_eval*42000, decimals=1)
    predict_array = predict.astype(type('float', (float,), {}))

    test = np.around(Y_test_eval*42000, decimals=1)
    ...
    l1 = (
        Line()
            ...
            title_opts=opts.TitleOpts(
                title="电量预测值与实际值对比图", subtitle="数据来自加拿大家庭用电数据集", pos_left="center"
            ),
            ...
            yaxis_opts=opts.AxisOpts(max_=42000, name="用电量"),
