...
def usa(self, status):
    ...
    def get_res_layer(output, last=False):
        ...
        if last:
            output_new = add([dense_res12, output], name='output')
        else:
            output_new = add([dense_res12, output])
        return output_new
    ...
    def get_model():
        model = Model(inputs=[input1_Dd, input1_Dw, input1_Dm, input1_Dr, input1_Td, input1_Tw, input1_Tm, input1_T,
                              ...
                              input10_Dd, input10_Dw, input10_Dm, input10_Dr, input10_Td, input10_Tw, input10_Tm,
                              input10_T,
    ...
    for i in range(NUM_REPEAT):
        for j in range(NUM_SNAPSHOTS):
            model.load_weights('complete' + str(i + 1) + str(j + 1) + '_weights_usa.h5')
    ...
    predict = np.around(pred_eval * 5000, decimals=1)
    predict_array = predict.astype(type('float', (float,), {}))

    test = np.around(Y_test_eval * 5000, decimals=1)
    ...
    timeData = [d.replace("1990/", "90/") for d in timeData]
    timeData = [d.replace("1991/", "91/") for d in timeData]
    timeData = [d.replace("1992/", "92/") for d in timeData]
    l1 = (
        Line()
            ...
            title_opts=opts.TitleOpts(
                title="电量预测值与实际值对比图", subtitle="数据来自北美公共事业数据集", pos_left="center"
            ),
            ...
            yaxis_opts=opts.AxisOpts(max_=5000, name="用电量"),
